...
import { getGoalDetail, updateGoal } from "../services/goal.service";
...
function GoalEditPage() {
    ...
    try {
      ...
      if (response.status === "SUCCESS") {
        ...
        setBasicInfo({
          ...
          period: goalData.period || 30,
          daily_duration: goalData.daily_duration || 3,
          participants_limit: goalData.participants_limit || 5,
          description: goalData.description || "",
        });

        // 중간 목표 설정
        if (goalData.mid_objectives && goalData.mid_objectives.length > 0) {
          setMidObjectives(goalData.mid_objectives);
        }

        // 주간 목표 설정
        if (
          goalData.weekly_objectives &&
          goalData.weekly_objectives.length > 0
        ) {
          setWeeklyObjectives(goalData.weekly_objectives);
        }

        // 할 일 목록 설정
        if (goalData.todo_list && goalData.todo_list.length > 0) {
          setTodoList(goalData.todo_list);
        }

        // 이미지 설정
        if (goalData.thumbnail_images && goalData.thumbnail_images.length > 0) {
          setThumbnailImages(goalData.thumbnail_images);
        }

        if (goalData.content_images && goalData.content_images.length > 0) {
          setContentImages(goalData.content_images);
        }
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 수정 API 호출 (실제 구현은 비워둠)
      console.log("수정할 데이터:", goalData);
  ...
  if (fetchLoading) return <LoadingSpinner size="big" text={true} />;
      ...
      <form onSubmit={handleSubmit} className="goal-form">
        ...
        <div className="form-actions">
          ...
          <button type="submit" className="submit-button" disabled={loading}>
            {loading ? <span className="loading-spinner-small"></span> : "수정"}
          </button>
        </div>
      </form>
    </div>
